...
def params(mySetParams):
    ...
    parser.add_argument('-t','--test_mode', help='Enter Mode Type: TEST or TRAIN', 
                        choices=[TRAIN, TEST], default=TRAIN, nargs='?', metavar="")
...
if __name__ == "__main__":
    possibleLayerSizes = [32, 64, 128, 32, 32, 32, 128]
    possibleLayerSizesT = [32, 64, 128, 64, 128, 64, 64]
    possibleLayerSizesTT = [32, 64, 128, 32, 32, 128, 32]

    myList = []
    for model in [SIMPLE, DOUBLE]:
        for network in [SIMPLE, DUELING]:
            for one, two, three in zip(possibleLayerSizes, possibleLayerSizesT, possibleLayerSizesTT):
                mySetParams = {
                ...
                myList.append(mySetParams)


    myCount = 0
    myFileParams = [len(myList),possibleLayerSizes,possibleLayerSizesT,possibleLayerSizesTT, myCount]
    toBeProcessed = myList
    if Path('checkpoint.json').is_file():
        with open('checkpoint.json') as data_file:
            data_loaded = json.load(data_file)

        if data_loaded[:-1] == myFileParams[:-1]:
            #same file restore from checkpoint
            myCount = data_loaded[-1]
            toBeProcessed = myList[myCount:]

    with open('checkpoint.json', 'w') as f:
        json.dump(myFileParams, f)
        


    ## run the program and save to json for each done
    for i in toBeProcessed:
        params(i)
        myCount+=1
        myFileParams  = [len(myList),possibleLayerSizes,possibleLayerSizesT,possibleLayerSizesTT, myCount] 
        print('Done with {}/{}'.format(myCount,len(myList)))
        with open('checkpoint.json', 'w') as f:
            json.dump(myFileParams, f)
